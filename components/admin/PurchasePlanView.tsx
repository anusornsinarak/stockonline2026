...
export const PurchasePlanView: React.FC<PurchasePlanViewProps> = ({ products = [], fiscalYear, currentFiscalYearBE, budget, aggregatedSurveyData, initialPlan, onPlanSave, inventory, documentSettings, productUsageHistory }) => {
    ...
    useEffect(() => {
        const fetchPlanForYear = async () => {
            ...
            try {
                const plan = await supabaseService.getPurchasePlan(selectedFiscalYear);
                setCurrentPlan(plan);
            } catch (error) {
    ...
    const planData = useMemo(() => {
        ...
        return products.map(product => {
            ...
            return { product, totalQuantity, plannedQty, currentStock, plannedValue, usageHistory };
        }).sort((a,b) => a.product.name.localeCompare(b.product.name, 'th'));
    }, [products, aggregatedSurveyData, plannedManualQuantities, inventoryMap, productUsageHistory]);
    ...
    const handleSave = async () => {
        ...
        try {
            await supabaseService.savePurchasePlan(selectedFiscalYear, itemsToSave);
    ...
    const renderTableRow = (item: any) => (
        <tr key={item.product.id} className="hover:bg-slate-50/70">
            ...
            <td className="px-6 py-2 text-sm text-right text-blue-600">{item.currentStock.toLocaleString()}</td>
